...
##Create G matrix with AGHmatrix package
  newData<-Data[,-c(1:2)]
  newData<-t(newData)
...
for (k in 1:11) {
  ...
  colnames(neweblueH1b)[2:3]<-c('eblue', 'std.error_eblue')
      ...
      modelharvest1<-asreml(fixed = DM_kg_ha ~ 1,
                            random = ~ Row + Column + vm(Juliana.Code2.x,GINV)
                            , 
   ...
   colnames(r1)<-c("Harvest","Male","Female","Ginv","pcor","mse")
     ...
     colnames(r1)<-c("Harvest","Male","Female","Ginv","pcor","mse")
   setwd("/Users/pablosipowicz/Documents/Forage Breeding Team/AGP17")
  write.csv(r1,paste0(hn,'_AGP17_unrelated_Full', '.csv'),quote=FALSE)
}
