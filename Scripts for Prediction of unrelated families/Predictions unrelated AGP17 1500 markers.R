...
##Make vectors with male and female names to select in Cross-validation 
...
####The l loop will use a different Gmatrix built from 500 markers (10 different Gmatrices by randomly selecting 50 markers)
...
for (k in 1:11) {
  ...
  colnames(r1)<-c("Harvest","Male","Female","Ginv","pcor","mse")
    ...
    mse<-mean((all_n$eblue-all_n$predicted.value)^2)
  ...
  write.csv(r1,paste0(hn,'_AGP17_unrelated_1500', '.csv'),quote=FALSE)
}
