...
#######Randomly select 50 markers (10 times) to estimate G matrix and create G matrix with AGHmatrix package
A<-list()
B<-list()
#C<-list()
#D<-list()
...
#  n <- Tot[c(Tot %not in% c(A[[i]],B[[i]],C[[i]],D[[i]],E[[i]],G[[i]]))]
  ...
  Check[[i]]<-length(unique(Total[[i]]))###This line is to check if there are actually 50 markers in each iteration
...
####The loop j will use a different Gmatrix built from 50 markers (10 different Gmatrices by randomly selecting 50 markers at)
for (k in 1:11) {
  hn <- k
...
for (i in 1:100) {
  ...
  for (j in 1:10) {
    ...
    setwd("/Users/pablosipowicz/Documents/Forage Breeding Team/AGP17/Genomic Selection/DaRT/AlleleFreq/GBLUP/New/50markers/Harvests")
    write.csv(metrics,paste0('./metrics10fold_DaRT_50markers_DMY_GBLUP',hn, '.csv'),quote=FALSE)
  }
}
}
