...
Data<-read.csv("/Users/pablosipowicz/Documents/Forage Breeding Team/AGP17/Genomic Selection/Haplotypes DArT/HaploFreq1_162_NoNA.csv")
...
mean(neweblueH1b$predicted.value)
newData<-Data
newData<-newData[,-c(1:3)]
newData<-t(newData)
rownames(newData)<-gsub("X","F",rownames(newData))
newData<-newData[rownames(newData) %in% Family,]
Gmatrix_BI<-Gmatrix(newData, method="VanRaden", ratio = TRUE, ploidy = 4)

Maleinfo<-male.info$Unique.Code
male.info$Unique.Code<-paste0("F",Maleinfo)
data2<-merge(male.info, data_byharv, by='Family')

GINV <- G.inverse(G=Gmatrix_BI, sparseform=TRUE,blend=T)$Ginv.sparse  
...
validation<-list()
predicted<-list()
metrics<-matrix(0,nrow = 100, ncol = 7)
...
for (i in 1:100) {
  ...
  genot_validation<-unique(val[[i]]$Juliana.Code2)
    ...
    GINV_j<-"Full"
    trn_fam_i<-i
    metrics[i,]<-c(paste0("H ",hn),GINV_j,pcor,pcor_spearman,mse,slope,trn_fam_i)
    ...
    setwd("/Users/pablosipowicz/Documents/Forage Breeding Team/AGP17/Genomic Selection/DaRT/Full/Harvests")
    write.csv(metrics,paste0('./metrics10fold_DaRT_Full_DMY_',hn, '.csv'),quote=FALSE)
  }
}
