...
#Create Gmatrix from random sampling of 1000 markers ten times
...
####The j loop will use a different Gmatrix built from 1000 markers (10 different Gmatrices by randomly selecting 50 markers at)
for (k in 1:11) {
  hn <- k
...
for (i in 1:100) {
  ...
  for (j in 1:10) {
    ...
    setwd("/Users/pablosipowicz/Documents/Forage Breeding Team/AGP17/Genomic Selection/Sequence Capture/1000markers/Harvests")
    write.csv(metrics,paste0('./metrics10fold_SeqCap1000markers_DMY_',hn, '.csv'),quote=FALSE)
  }
}
}
